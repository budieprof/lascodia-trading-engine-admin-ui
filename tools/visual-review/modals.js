...
const STEPS = [
  ...
  { url: '/prompt-templates', name: 'prompt-fork-modal', act: async p => { await p.getByRole('button', { name: /^fork$/i }).first().click(); } },
  { url: '/spread-reactive', name: 'spread-floor-override', act: async p => { await p.getByRole('button', { name: /^baseline floors$/i }).first().click(); await p.waitForTimeout(800); await p.getByRole('button', { name: /^\+ new override$/i }).first().click(); } },
...
(async () => {
  ...
  await ctx.addInitScript(({ token, user }) => { sessionStorage.setItem('lascodia.auth.token', token); sessionStorage.setItem('lascodia.auth.user', JSON.stringify(user)); }, { token, user });
