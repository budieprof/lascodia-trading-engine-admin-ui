...
const colorArr = (v: unknown): (string | null)[] | null =>
...
export function normalizeRunResult(raw: unknown): PineRunResult | null {
  ...
  if ('status' in raw && 'data' in raw && isObj(raw['data']))
    return normalizeRunResult(raw['data']);
  return {
...
export function normalizeCompile(raw: unknown): PineCompileResult | null {
  ...
  return {
    ...
    diagnostics: arr<Obj>(raw['diagnostics'])
      .filter(isObj)
      .map((d) => ({
        code: str(d['code'], ''),
...
export function normalizeOutputs(raw: unknown): PineScriptOutputs | null {
  ...
  return {
    ...
    fills: arr<Obj>(raw['fills'])
      ...
      .map(
        (f): PineFillOutput => ({
          ...
          bottomValues: Array.isArray(f['bottomValues'])
            ? (f['bottomValues'] as (number | null)[])
            : null,
          topColors: colorArr(f['topColors']),
          bottomColors: colorArr(f['bottomColors']),
        }),
      ),
    labels: arr<Obj>(raw['labels'])
      .filter(isObj)
      .map(
        (l): PineLabelOutput => ({
          id: num(l['id'], 0),
    ...
    polylines: arr<Obj>(raw['polylines'])
      .filter(isObj)
      .map(
    ...
    linefills: arr<Obj>(raw['linefills'])
      .filter(isObj)
      .map(
    ...
    alertConditions: arr<Obj>(raw['alertConditions'])
      .filter(isObj)
      .map((a) => ({
        id: num(a['id'], 0),
