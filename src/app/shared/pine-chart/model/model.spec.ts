...
import { normalizeBars, normalizeOutputs, normalizeReplayStart, normalizeRunResult } from './normalize';
import type { PineScriptOutputs } from './pine-outputs.types';

describe('normalize', () => {
  it('fills Pine defaults for everything the engine leaves out', () => {
    const out = normalizeOutputs({ bars: { firstIndex: 3, times: [1, 2] }, plots: [{ id: 1, values: [1, null] }], labels: [{ id: 9, x: { barIndex: 4 } }] })!;
    expect(out.plots[0]).toMatchObject({ style: 'line', lineStyle: 'solid', lineWidth: 1, display: ['all'], offset: 0, forceOverlay: false });
    expect(out.labels[0]).toMatchObject({ style: 'label_down', yloc: 'price', xloc: 'bar_index', x: { barIndex: 4, time: null } });
  ...
  it('unwraps a ResponseData envelope and sorts the trace', () => {
    const run = normalizeRunResult({
      status: true,
      data: { compile: { success: true, diagnostics: [], declaration: { title: 'X', overlay: true } }, bars: [], trace: [{ bar: 5, items: [] }, { bar: 2, items: [{ line: 1, value: null }] }] },
    })!;
  ...
  it('accepts bars in either shape', () => {
    ...
    expect(normalizeBars([{ time: 1, open: 1, high: 2, low: 0, close: 1.5 }])).toEqual([{ t: 1, o: 1, h: 2, l: 0, c: 1.5, v: 0 }]);
    expect(normalizeBars('nope')).toEqual([]);
  });

  it('reads replay starts with numeric or string session ids', () => {
    expect(normalizeReplayStart({ sessionId: 42, frame: { barIndex: 10, bars: [] } })?.sessionId).toBe('42');
  ...
  it('toPineChartData takes a run result or chart data', () => {
    ...
    const b = toPineChartData({ bars: run.bars, outputs: run.outputs, report: null, declaration: { title: 'D', overlay: false } })!;
...
describe('mergeOutputs (Bar Replay)', () => {
  const perBarOf = (o: PineScriptOutputs) => ({
    times: o.bars.times,
    plots: o.plots.map((p) => ({ id: p.id, values: p.values, colors: p.colors ?? o.bars.times.map(() => p.color ?? null) })),
    ...
    fills: o.fills.map((f) => ({ top: f.topValues, colors: f.colors ?? o.bars.times.map(() => f.color ?? null) })),
  });
  ...
  it('keeps a uniform color uniform and expands it only when colors diverge', () => {
    const base = normalizeOutputs({ bars: { firstIndex: 0, times: [1, 2] }, plots: [{ id: 0, values: [1, 2], color: '#FF0000FF' }] })!;
    const same = normalizeOutputs({ bars: { firstIndex: 2, times: [3] }, plots: [{ id: 0, values: [3], color: '#FF0000FF' }] })!;
    const other = normalizeOutputs({ bars: { firstIndex: 2, times: [3] }, plots: [{ id: 0, values: [3], color: '#0000FFFF' }] })!;
    expect(mergeOutputs(base, same)!.plots[0]).toMatchObject({ color: '#FF0000FF', colors: null, values: [1, 2, 3] });
    expect(mergeOutputs(base, other)!.plots[0].colors).toEqual(['#FF0000FF', '#FF0000FF', '#0000FFFF']);
  });
  ...
  it('adds an output that first appears in a later frame, padded with na', () => {
    ...
    const delta = normalizeOutputs({ bars: { firstIndex: 2, times: [3] }, plots: [{ id: 4, values: [7] }] })!;
