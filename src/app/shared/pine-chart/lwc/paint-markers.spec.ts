...
const proj = () => linearProjection({ barSpacing: 10, y0: 200, pxPerUnit: -1, width: 400, height: 300, from: -1, to: 60, lastLogical: 50 });
...
const circleCenters = (ctx: RecordingContext) =>
  ctx.fills().flatMap((f) => f.path.filter((c): c is { c: 'A'; x: number; y: number; r: number } => c.c === 'A'));
...
describe('paintMarkers', () => {
  ...
  it('places abovebar over the high, belowbar under the low', () => {
    ...
    paintMarkers(ctx.asCtx(), proj(), markerLayer({ location: 'belowbar' }), bars, new MarkerStacks());
  ...
  it('top/bottom hug the pane edges; absolute sits on the value', () => {
    ...
    paintMarkers(ctx.asCtx(), proj(), markerLayer({ location: 'bottom' }), bars, new MarkerStacks());
    paintMarkers(ctx.asCtx(), proj(), markerLayer({ location: 'absolute', values: Float64Array.of(150) }), bars, new MarkerStacks());
  ...
  it('writes plotshape text above an abovebar shape and inside a label shape', () => {
    ...
    paintMarkers(bubble.asCtx(), proj(), markerLayer({ shape: 'labeldown', text: 'Sell' }), bars, new MarkerStacks());
  ...
  it('plotchar draws its character', () => {
    const ctx = new RecordingContext();
    paintMarkers(ctx.asCtx(), proj(), markerLayer({ kind: 'char', char: '▲' }), bars, new MarkerStacks());
  ...
  it('plotarrow: up under the bar, down over it, length proportional to |value|', () => {
    ...
    const layer = markerLayer(
      { kind: 'arrow', minHeight: 10, maxHeight: 50, values: Float64Array.of(2, -1), up: Uint8Array.of(1, 0), colors: ['rgb(0, 128, 0)', 'rgb(255, 0, 0)'] },
      [2, 4],
    );
    ...
    const span = (pts: Array<[number, number]>) => Math.max(...pts.map((p) => p[1])) - Math.min(...pts.map((p) => p[1]));
...
describe('strategy trades', () => {
  ...
  it('a long trade buys in (blue, up, +qty) and sells out (purple, down, −qty)', () => {
    ...
    expect(entry).toMatchObject({ side: 'buy', kind: 'entry', color: TRADE_COLORS.longEntry, text: 'Long\n+2', logical: 5, price: 110 });
    expect(exit).toMatchObject({ side: 'sell', kind: 'exit', color: TRADE_COLORS.exit, text: 'TP\n-2', logical: 12 });
  ...
  it('a short trade sells in (red) and buys out', () => {
    ...
    expect(entry).toMatchObject({ side: 'sell', color: TRADE_COLORS.shortEntry, text: 'Short\n-2' });
  ...
  it('draws the dashed entry→exit line in the profit color, arrows and hit regions', () => {
    ...
    paintTrades(ctx.asCtx(), proj(), [trade(), trade({ number: 2, entryX: 20, exitX: 25, profit: -3, lineColor: TRADE_COLORS.loss })], bars, 118, new MarkerStacks(), hits);
    ...
    expect(ctx.texts().map((t) => t.text)).toEqual(expect.arrayContaining(['Long', '+2', 'TP', '-2']));
    expect(hits).toHaveLength(4);
  });

  it('an open trade is drawn to the last bar at the last close', () => {
    const ctx = new RecordingContext();
    paintTrades(ctx.asCtx(), proj(), [trade({ isOpen: true, exitX: null, exitPrice: null })], bars, 118, new MarkerStacks(), []);
