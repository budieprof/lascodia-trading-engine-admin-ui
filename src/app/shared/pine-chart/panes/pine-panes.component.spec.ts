...
declareSignalIo(PineLogsPaneComponent, { inputs: ['logs', 'droppedLogs', 'runtimeError', 'timezone'], outputs: ['barJump', 'lineJump'] });
declareSignalIo(PineTracePaneComponent, { inputs: ['trace', 'bar', 'timezone', 'windowSize'], outputs: ['barChange', 'lineJump', 'requestTrace'] });
declareSignalIo(PineProfilerPaneComponent, { inputs: ['profile', 'source', 'elapsedMs'], outputs: ['lineJump'] });

afterEach(() => TestBed.resetTestingModule());

function create<T>(type: new (...a: never[]) => T, inputs: Record<string, unknown>): ComponentFixture<T> {
...
const $ = (f: ComponentFixture<unknown>, sel: string) => f.nativeElement.querySelector(sel) as HTMLElement | null;
const $$ = (f: ComponentFixture<unknown>, sel: string) => [...f.nativeElement.querySelectorAll(sel)] as HTMLElement[];

const T = Date.UTC(2026, 8, 1);
const logs: PineLogOutput[] = [
  { level: 'info', message: 'close=1.1 fast=1.09', barIndex: 3, time: T, isRealtime: false, line: 20 },
  { level: 'warning', message: 'Wide bar\nrange 0.004', barIndex: 7, time: T + 3_600_000, isRealtime: false },
  { level: 'error', message: 'Order rejected', barIndex: 9, time: T + 7_200_000, isRealtime: false },
];
...
describe('PineLogsPaneComponent', () => {
  it('lists every log with its level, ISO time and first line', () => {
    ...
    const rows = $$(f, '.row');
    expect(rows.map((r) => r.className.match(/info|warning|error/)?.[0])).toEqual(['info', 'warning', 'error']);
  ...
  it('shows a runtime error banner with its line and bar', () => {
    const f = create(PineLogsPaneComponent, { logs: [], runtimeError: { code: 'PS5003', message: 'Division by zero', line: 12, column: 5, barIndex: 400 } });
    const lines: PineLineJump[] = [];
    const bars: number[] = [];
    f.componentInstance.lineJump.subscribe((l) => lines.push(l));
    f.componentInstance.barJump.subscribe((b) => bars.push(b));
...
describe('PineTracePaneComponent', () => {
  const trace: PineTraceBar[] = [100, 101, 102].map((bar) => ({
    ...
    items: [{ line: 9, column: 6, endLine: 9, endColumn: 30, text: 'ta.crossover(fast, slow)', value: bar === 101 ? 'true' : 'false' }],
  }));
...
describe('PineProfilerPaneComponent', () => {
  ...
  it('lists lines by time with heat bars and flames on the top three', () => {
    const f = create(PineProfilerPaneComponent, { profile, source: 'a\nb\nr = ta.rsi(close, 14)\nd\ne\nf' });
    const rows = $$(f, '.tr');
    expect(rows.map((r) => r.querySelector('.num')?.textContent?.trim())).toEqual(['3', '5', '4', '6']);
  ...
  it('sorts by a column and jumps to a line', () => {
    ...
    lineHeader.click();
    f.detectChanges();
    expect($$(f, '.tr').map((r) => r.querySelector('.num')?.textContent?.trim())).toEqual(['3', '4', '5', '6']);
    const lines: PineLineJump[] = [];
    f.componentInstance.lineJump.subscribe((l) => lines.push(l));
    $$(f, '.tr')[3].click();
