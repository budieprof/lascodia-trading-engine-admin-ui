...
/** What each built-in namespace is for — hover docs for `ta`, `strategy`, … */
export const PINE_NAMESPACE_DOCS: Readonly<Record<string, string>> = {
  ta: 'Technical-analysis functions: moving averages, oscillators, crosses, highest/lowest, pivots. Call them on every bar.',
  math: 'Mathematical functions and constants.',
  str: 'String functions: formatting, searching, splitting, conversion.',
  array: 'Functions of the `array<T>` collection (most are also methods: `arr.push(x)`).',
  matrix: 'Functions of the `matrix<T>` collection.',
  map: 'Functions of the `map<K, V>` collection.',
  color: 'Colour constants and functions: `color.new()`, `color.rgb()`, gradients.',
  input:
    'Inputs shown in the strategy settings: `input.int()`, `input.float()`, `input.source()`, …',
  strategy:
    'Strategy orders (`strategy.entry/exit/close`), settings constants and position/trade state.',
  'strategy.risk':
    'Strategy risk rules: max drawdown, intraday loss, position size, allowed direction.',
  'strategy.closedtrades': 'Closed-trade details by trade number.',
  'strategy.opentrades': 'Open-trade details by trade number.',
  'strategy.commission': 'Commission types for `strategy(commission_type = …)`.',
  'strategy.direction': 'Directions for `strategy.risk.allow_entry_in()`.',
  'strategy.oca': 'One-cancels-all group behaviours.',
  request:
    'Data from other symbols, timeframes and sources: `request.security()`, `request.security_lower_tf()`, …',
  ticker: 'Ticker identifiers, including synthetic charts (Heikin-Ashi, Renko, …).',
  timeframe: 'The chart timeframe and timeframe arithmetic.',
  session: 'Session state of the current bar and session constants.',
  syminfo: 'Information about the chart symbol: tick size, currency, session, …',
  barstate: 'Where the current bar is in the dataset: first, last, realtime, confirmed, …',
  chart: 'Chart properties and `chart.point` coordinates.',
  label: 'Label drawings.',
  line: 'Line drawings.',
  box: 'Box drawings.',
  table: 'Tables drawn over the chart.',
  polyline: 'Polyline drawings.',
  linefill: 'Fills between two lines.',
  log: 'Pine logs: `log.info()`, `log.warning()`, `log.error()`.',
  runtime: 'Runtime control: `runtime.error()`.',
  alert: 'Alert frequency constants for `alert()`.',
  plot: 'Plot style constants.',
  shape: 'Shapes for `plotshape()`.',
  location: 'Locations for `plotshape()` / `plotchar()`.',
  size: 'Sizes for shapes, labels and text.',
  position: 'Table positions.',
  display: 'Where a plot or input value is displayed.',
  format: 'Number formats for the script display.',
  barmerge: 'Gap and lookahead behaviours for `request.security()`.',
  currency: 'Currency codes.',
  dayofweek: 'Day-of-week constants.',
  xloc: 'Horizontal coordinate modes of drawings (bar index / bar time).',
  yloc: 'Vertical coordinate modes of labels.',
  extend: 'Line extension modes.',
  hline: 'Line styles for `hline()`.',
  text: 'Text alignment, wrapping and formatting constants.',
  font: 'Font families.',
  order: 'Sort orders.',
  scale: 'Price scale placement.',
};
